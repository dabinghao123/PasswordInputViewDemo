...
UITapGestureRecognizer     *_viewTopGes;
...
+ (void)translucentCoverFrom:(UIView *)fromView content:(UIView *)contentView animated:(BOOL)animated notClick:(BOOL)notClick showBlock:(showBlock)showBlock hideBlock:(hideBlock)hideBlock
{
    ...
    // 如果遮罩能点
    if (!notClick) {
        _viewTopGes = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(hide)];
        [cover addGestureRecognizer:_viewTopGes];
    }
...
//        [cover addGestureRecognizer:[[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(hide)]];
        ...
        [cover removeGestureRecognizer:_viewTopGes];
    }
...
+(void)prohibitCoverTop
{   
    _viewTopGes.numberOfTapsRequired = 3;
    _viewTopGes.numberOfTouchesRequired = 3;
}
+(void)enableCoverTop
{
    _viewTopGes.numberOfTapsRequired = 1;
    _viewTopGes.numberOfTouchesRequired = 1;
}
